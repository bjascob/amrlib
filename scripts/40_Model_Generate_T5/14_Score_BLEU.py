...
# Read space-tokenized file with 1 sentence per line
def read_file(fname):
    print('Reading ', fname)
    sents = []
...
# Take 2 files with 1 sentence per line, space-tokenized, and load them.
# Run the tokenized sentences through the BLEU scorer.
if __name__ == '__main__':
    model_dir = 'amrlib/data/model_generate_t5'

    golden_file = os.path.join(model_dir, 'test.txt.ref_sents')
    pred_file   = os.path.join(model_dir, 'test.txt.generated')
