...
class AMRBartTokenizer(BartTokenizer):
    ...
    def init_amr_vocabulary(self, pred_min=5):
        ...
        tokens = []
        for line in Path(res_dir, 'predicates.txt').read_text().strip().splitlines():
            tok, count = line.split()
            if int(count) >= pred_min:
                tokens.append(tok)

        for tok in Path(res_dir, 'additions.txt').read_text().strip().splitlines():
            tokens.append(tok)

        for tok in Path(res_dir, 'recategorizations.txt').read_text().strip().splitlines():
    ...
    def batch_encode_graphs_from_linearized(self, linearized, extras=None, device=torch.device('cpu')):
        ...
        # Looks like slicing makes 'labels' non-contiguous and contiguous slicing is required in model.forward()
        batch = {'decoder_input_ids': batch[:, :-1].contiguous(), 'labels': batch[:, 1:].contiguous()}
    ...
    def decode_amr(self, tokens, restore_name_ops=False):
        ...
        try:
            ...
            logger.warning('Building failure:\n%s\n%s\n%s' % (nodes, backreferences, e))
            return postprocessing.BACKOFF, postprocessing.ParsedStatus.BACKOFF, (None, None)
        try:
            graph, status = postprocessing.connect_graph_if_not_connected(graph)
            if status == postprocessing.ParsedStatus.BACKOFF:
                logger.warning('Reconnection 1 failure: %s\n%s\n%s' % (nodes, backreferences, graph_))
            return graph, status, (nodes, backreferences)
        except Exception as e:
            logger.warning('Reconnction 2 failure: %s\n%s\n%s\n%s' % (e, nodes, backreferences, graph_))
...
class PENMANBartTokenizer(AMRBartTokenizer):
    ...
    def _fix_and_make_graph(self, nodes):
        ...
        linearized = penman.encode(graph, model=amr_model)
    ...
    def decode_amr(self, tokens, restore_name_ops=None):
        ...
        try:
            ...
            logger.warning('Building failure: %s\n%s\n%s' % (nodes, backreferences, e))
            return postprocessing.BACKOFF, postprocessing.ParsedStatus.BACKOFF, (None, None)
        try:
            graph, status = postprocessing.connect_graph_if_not_connected(graph)
            if status == postprocessing.ParsedStatus.BACKOFF:
                logger.warning('Reconnection 1 failure: %s\n%s\n%s\n' % (nodes, backreferences, graph_))
