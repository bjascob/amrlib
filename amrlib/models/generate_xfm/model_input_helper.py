...
# Notes: The trainer uses load_amr_graph_sent to load the training data so this should work but
#        these graphs are not AMR compliant and penman will not load / parse these correctly.
class ModelInputHelper(object):
    ...
    def __init__(self, graph, reannotate=False):
        ...
        is_annotated = all([key in pgraph.metadata for key in ('pos_tags',)])
        if not is_annotated or reannotate:
